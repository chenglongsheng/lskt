    ...
    <el-table :data="list" border stripe>
      ...
      <el-table-column label="课程信息">
        <template slot-scope="scope">
          ...
          <p>分类：{{ scope.row.param.subjectParentTitle }} > {{ scope.row.param.subjectTitle }}</p>
          <p>
      ...
      <el-table-column label="讲师" width="100" align="center">
        <template slot-scope="scope">{{ scope.row.param.teacherName }}</template>
      </el-table-column>
      ...
      <el-table-column label="发布时间" width="140" align="center">
        <template
          slot-scope="scope"
        >{{ scope.row.createTime ? scope.row.createTime.substr(0, 16) : '' }}</template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    initSubjectList() {
      subjectApi.getChildren(0).then(response => {
        this.subjectList = response.data
      })
    },

    subjectLevelOneChanged(value) {
      subjectApi.getChildren(value).then(response => {
